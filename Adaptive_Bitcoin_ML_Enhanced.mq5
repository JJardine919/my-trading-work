//+------------------------------------------------------------------+
//|                     Adaptive_Bitcoin_ML_Enhanced.mq5            |
//|                                  Copyright 2025, SUSTAI Trading |
//|                                                                 |
//| ML-enhanced version with balanced ONNX trend predictions        |
//| Uses cluster-centroids resampling for unbiased bull/bear trading|
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, SUSTAI Trading"
#property link      ""
#property version   "2.00"
#property strict
#property description "ML-Enhanced Bitcoin bot with balanced trend detection"

// Include Random Forest classifier for ONNX models
#include <Random Forest.mqh>
CRandomForestClassifier g_trendModel;
...
// Input Parameters - ML Settings
input string   ML_Settings = "--- ML Trend Detection Settings ---";
input bool     UseMLTrendDetection = true;     // Use ML-based trend detection (vs simple EMA)
input string   MLModelName = "BTCUSD.PERIOD_D1.cluster-centroids.onnx";  // ONNX model filename
input double   MLConfidenceThreshold = 0.6;    // Minimum ML prediction confidence (0.5-1.0)
input ENUM_TIMEFRAMES MLTimeframe = PERIOD_D1; // Timeframe for ML trend analysis
...
string BotName = "Adaptive_Bitcoin_ML_Enhanced";
...
int OnInit()
{
    ...
    // Initialize ML trend detection model
    MLModelLoaded = false;
    if(UseMLTrendDetection)
    {
        Print("Loading ML trend detection model: ", MLModelName);
        if(g_trendModel.Init(MLModelName, ONNX_COMMON_FOLDER))
        {
            MLModelLoaded = true;
            Print("✓ ML model loaded successfully! Using balanced trend predictions.");
        }
        else
        {
            Print("✗ ML model failed to load. Falling back to traditional EMA trend detection.");
            MLModelLoaded = false;
        }
    }
    else
    {
        Print("ML trend detection disabled. Using traditional EMA crossover.");
    }
    ...
    Print("ML Trend Detection: ", UseMLTrendDetection ? "ENABLED" : "DISABLED",
          " (Model: ", MLModelLoaded ? "LOADED" : "NOT LOADED", ")");
...
int DetermineMarketTrend()
{
    ...
    // Use ML-based trend detection if enabled and model is loaded
    if(UseMLTrendDetection && MLModelLoaded)
    {
        return DetermineMarketTrendML();
    }

    // Fallback to traditional EMA crossover
    return DetermineMarketTrendEMA();
}

//+------------------------------------------------------------------+
//| ML-based trend detection using balanced ONNX model               |
//+------------------------------------------------------------------+
int DetermineMarketTrendML()
{
    // Get OHLC data from previous candle on ML timeframe
    vector features;
    features.Init(4);

    features[0] = iOpen(CurrentSymbol, MLTimeframe, 1);
    features[1] = iHigh(CurrentSymbol, MLTimeframe, 1);
    features[2] = iLow(CurrentSymbol, MLTimeframe, 1);
    features[3] = iClose(CurrentSymbol, MLTimeframe, 1);

    // Validate data
    if(features[0] <= 0 || features[1] <= 0 || features[2] <= 0 || features[3] <= 0)
    {
        if(!IsTestingMode) Print("Invalid OHLC data for ML prediction, falling back to EMA");
        return DetermineMarketTrendEMA();
    }

    // Get ML prediction: 1 = Bullish (price will rise), 0 = Bearish (price will fall)
    long prediction = g_trendModel.predict_bin(features);

    // Get prediction confidence (probability)
    vector probabilities = g_trendModel.predict_proba(features);
    double confidence = probabilities.Max();

    // Log ML prediction (only in live mode for debugging)
    if(!IsTestingMode)
    {
        Print("ML Prediction: ", (prediction == 1 ? "BULLISH" : "BEARISH"),
              " (confidence: ", DoubleToString(confidence * 100, 1), "%)");
    }

    // Only use ML prediction if confidence exceeds threshold
    if(confidence < MLConfidenceThreshold)
    {
        if(!IsTestingMode)
            Print("ML confidence ", DoubleToString(confidence * 100, 1),
                  "% below threshold ", DoubleToString(MLConfidenceThreshold * 100, 1),
                  "%, using sideways signal");
        return 0; // Sideways/neutral when confidence is low
    }

    // Return trend based on ML prediction
    // prediction == 1 means price will rise (bullish/uptrend)
    // prediction == 0 means price will fall (bearish/downtrend)
    return (prediction == 1) ? 1 : -1;
}

//+------------------------------------------------------------------+
//| Traditional EMA-based trend detection (fallback)                 |
//+------------------------------------------------------------------+
int DetermineMarketTrendEMA()
{
