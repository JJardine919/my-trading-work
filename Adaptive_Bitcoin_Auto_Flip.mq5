//+------------------------------------------------------------------+
//|                     Adaptive_Bitcoin_Auto_Flip.mq5              |
//|                                  Copyright 2025, SUSTAI Trading |
//|                                                                 |
//| AUTO-ADAPTIVE version that flips indicator levels               |
//| when market switches bull/bear - NO MANUAL ADJUSTMENT NEEDED    |
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, SUSTAI Trading"
#property link      ""
#property version   "2.00"
#property strict
#property description "Auto-adaptive Bitcoin bot - flips RSI/Stoch/MFI/EMA on trend change"
...
// Input Parameters - Auto-Adaptive Settings
input string   AutoAdaptive_Settings = "--- AUTO-ADAPTIVE SETTINGS ---";
input bool     EnableAutoFlip = true;          // Enable auto-flip on trend change
input ENUM_TIMEFRAMES AdaptiveTrendTimeframe = PERIOD_H4;  // Timeframe for trend detection

// Input Parameters - Indicator Settings (will auto-flip if EnableAutoFlip=true)
input string   RSI_Settings = "--- RSI Settings (auto-flips 70/30 ↔ 30/70) ---";
input int      RSI_Period = 14;                // RSI period
input double   RSI_MiddleLevel = 50.0;         // RSI middle level

input string   Stoch_Settings = "--- Stochastic Settings (auto-flips 80/20 ↔ 20/80) ---";
...
input string   MFI_Settings = "--- MFI Settings (auto-flips 80/20 ↔ 20/80) ---";
input int      MFI_Period = 14;                // MFI period

input string   MA_Settings = "--- Moving Average Settings (auto-flips 12/26 ↔ 26/12) ---";
// Fast and Slow EMA periods will be set dynamically based on trend
...
string BotName = "Adaptive_Bitcoin_Auto_Flip";
...
//--- Dynamic Adaptive Parameters (auto-flip based on trend)
double RSI_UpperLevel = 70.0;
double RSI_LowerLevel = 30.0;
double Stoch_UpperLevel = 80.0;
double Stoch_LowerLevel = 20.0;
double MFI_UpperLevel = 80.0;
double MFI_LowerLevel = 20.0;
int Fast_EMA_Period = 12;
int Slow_EMA_Period = 26;
int CurrentTrendDirection = 0;  // 1=bull, -1=bear, 0=unknown
...
int OnInit()
{
    ...
    // Initialize adaptive parameters (detect initial trend)
    CurrentTrendDirection = 0;
    int initialTrend = DetermineMarketTrend();  // This will set the initial parameters
    ...
    // Log auto-flip status
    if(EnableAutoFlip)
    {
        Print("🔄 AUTO-FLIP ENABLED - Parameters will adapt to trend changes");
        Print("   Trend detection: ", EnumToString(AdaptiveTrendTimeframe), " timeframe");
        Print("   Current trend: ", (initialTrend == 1 ? "BULLISH" : (initialTrend == -1 ? "BEARISH" : "SIDEWAYS")));
        Print("   RSI: ", RSI_UpperLevel, "/", RSI_LowerLevel,
              " | Stoch: ", Stoch_UpperLevel, "/", Stoch_LowerLevel,
              " | MFI: ", MFI_UpperLevel, "/", MFI_LowerLevel,
              " | EMA: ", Fast_EMA_Period, "/", Slow_EMA_Period);
    }
    else
    {
        Print("⚠️ AUTO-FLIP DISABLED - Using static parameters");
    }
...
//+------------------------------------------------------------------+
//| Update adaptive parameters based on trend                        |
//+------------------------------------------------------------------+
void UpdateAdaptiveParameters(int trendDirection)
{
    if(!EnableAutoFlip) return;  // Skip if auto-flip disabled

    if(trendDirection == CurrentTrendDirection) return;  // No change

    CurrentTrendDirection = trendDirection;

    if(trendDirection == 1)  // BULLISH - use 70/30, 80/20, 12/26
    {
        RSI_UpperLevel = 70.0;
        RSI_LowerLevel = 30.0;
        Stoch_UpperLevel = 80.0;
        Stoch_LowerLevel = 20.0;
        MFI_UpperLevel = 80.0;
        MFI_LowerLevel = 20.0;
        Fast_EMA_Period = 12;
        Slow_EMA_Period = 26;

        if(!IsTestingMode) Print("📈 BULLISH trend detected - Parameters set to 70/30, 80/20, 12/26");
    }
    else if(trendDirection == -1)  // BEARISH - FLIP to 30/70, 20/80, 26/12
    {
        RSI_UpperLevel = 30.0;      // FLIPPED
        RSI_LowerLevel = 70.0;      // FLIPPED
        Stoch_UpperLevel = 20.0;    // FLIPPED
        Stoch_LowerLevel = 80.0;    // FLIPPED
        MFI_UpperLevel = 20.0;      // FLIPPED
        MFI_LowerLevel = 80.0;      // FLIPPED
        Fast_EMA_Period = 26;       // FLIPPED
        Slow_EMA_Period = 12;       // FLIPPED

        if(!IsTestingMode) Print("📉 BEARISH trend detected - Parameters FLIPPED to 30/70, 20/80, 26/12");
    }
    else  // SIDEWAYS - use default bullish settings
    {
        RSI_UpperLevel = 70.0;
        RSI_LowerLevel = 30.0;
        Stoch_UpperLevel = 80.0;
...
int DetermineMarketTrend()
{
    ...
    // Get trend timeframe data (use FIXED 12/26 for detection, not flipped values)
    double fastEMA[], slowEMA[];
    ArraySetAsSeries(fastEMA, true);
    ArraySetAsSeries(slowEMA, true);

    if(CopyBuffer(iMA(CurrentSymbol, AdaptiveTrendTimeframe, 12, 0, MODE_EMA, PRICE_CLOSE), 0, 0, 3, fastEMA) <= 0) return 0;
    if(CopyBuffer(iMA(CurrentSymbol, AdaptiveTrendTimeframe, 26, 0, MODE_EMA, PRICE_CLOSE), 0, 0, 3, slowEMA) <= 0) return 0;
    ...
    {
        detectedTrend = 1; // Uptrend
    }
    else if(fastEMA[0] < slowEMA[0] && fastEMA[1] < slowEMA[1])
    {
        detectedTrend = -1; // Downtrend
    }

    // Update adaptive parameters if trend changed
    UpdateAdaptiveParameters(detectedTrend);
