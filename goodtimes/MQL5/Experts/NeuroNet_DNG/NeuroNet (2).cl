...
#define l1 1.0e-3f
#define l2 1.0e-3f
#define MAX_WEIGHT 1.0e-2f
#define MAX_GRAD 1.0e-4f
#define LOCAL_ARRAY_SIZE 64
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
float IsNaNOrInf(const float value, const float def_value)
  {
   ...
   for(int k = 0; k < outputs; k += 4)
     {
      ...
      //---
      weight.s0 = IsNaNOrInf(weight.s0, 0);
      weight.s1 = IsNaNOrInf(weight.s1, 0);
      weight.s2 = IsNaNOrInf(weight.s2, 0);
      weight.s3 = IsNaNOrInf(weight.s3, 0);
      grad.s0 = IsNaNOrInf(grad.s0, 0);
      grad.s1 = IsNaNOrInf(grad.s1, 0);
      grad.s2 = IsNaNOrInf(grad.s2, 0);
      grad.s3 = IsNaNOrInf(grad.s3, 0);
      //---
      sum += dot(grad, weight);
     }
//---
         ...
         switch(stop - k)
           {
            case 0:
               inp = (float4)(1, 0, 0, 0);
               weight = (float4)(matrix_w[shift_var_w + shift + window_in], 0, 0, 0);
               break;
            case 1:
               inp = (float4)(matrix_i[shift_var_in + shift_in + k], 1, 0, 0);
               weight = (float4)(matrix_w[shift_var_w + shift + k], matrix_w[shift_var_w + shift + window_in], 0, 0);
               break;
            case 2:
               inp =
...
//---
   float inp = fmax(IsNaNOrInf(inputs[shift_head + l], -100.0f), -100.0f);
//---
   for(int i = 0; i < total; i += ls)
     {
      if(l >= i && l < (i + ls))
         temp[l % ls] = (i > 0 ? fmax(inp, temp[l % ls]) : inp);
      barrier(CLK_LOCAL_MEM_FENCE);
     }
//---
...
//---
   for(int i = 0; i < total; i += ls)
     {
      if(l >= i && l < (i + ls))
         temp[l % ls] = (i > 0 ? temp[l % ls] : 0) + inp_exp;
      barrier(CLK_LOCAL_MEM_FENCE);
     }
//---
...
//---
   float grad = outputs_gr[shift_output];
   ...
   for(int s = 1; s <= smoothing; s++)
     {
      ...
      float factor = 1.0f / (min((int)total, (int)(pos + s)) - max((int)(pos - s), 0) + 1);
      //---
      float value = outputs_gr[shift] * factor;
      if(!(isinf(value) || isnan(value)))
         grad += value;
      //---
      if((pos - s) >= 0)
        {
         value = outputs_gr[shift - s * step_output] * factor;
         if(!(isinf(value) || isnan(value)))
            grad += value;
        }
      //---
