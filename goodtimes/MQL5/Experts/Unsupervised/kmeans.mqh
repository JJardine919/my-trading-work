...
public:
   ...
   bool              Statistic(CBufferDouble *data, CBufferDouble *targets);
   double            GetLoss(CBufferDouble *data);
   CBufferDouble     *GetProbability(CBufferDouble *data);
   //---
...
//|                                                                  |
//+------------------------------------------------------------------+
bool CKmeans::Clustering(CBufferDouble *data)
  {
...
//---
   ...
   if(!c_aClasters.BufferCreate(c_OpenCL) ||
      !targets.BufferCreate(c_OpenCL) ||
...
//---
   ...
   m_bTrained = true;
//---
   if(FileIsEnding(file_handle) || FileReadInteger(file_handle) == 0)
      return true;
//---
   if(CheckPointer(c_aProbability) == POINTER_INVALID)
     {
   ...
   if(FileIsEnding(file_handle) ||
      !c_aProbability.Load(file_handle))
      return false;
//---
   return true;
  }
